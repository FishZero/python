...
def main():
    ...
    # 备份母包部分资源
    _src = os.path.join(defaultApk, 'assets/res/webapphtml')
    _dst = os.path.join(backupDir, 'webapphtml')
    ...
    for index in config.autoPackageIndex:
        ...
        if os.path.exists(replaceDir):   
            ...
            #替换webapphtml目录
            _src = os.path.join(replaceDir, 'webapphtml')
            if not os.path.isdir(_src):
                _src = os.path.join(backupDir, 'webapphtml')
            _dst = os.path.join(defaultApk, 'assets/res/webapphtml')
            shutil.copytree(_src,_dst)
